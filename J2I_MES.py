# VERSÃO 5.26: otimização de leitura do Excel + tela de apontamento manual mais leve.
...
def localizar_colunas(ws) -> dict[str, int]:
    ...
    Usa leitura sequencial da primeira linha. Isso funciona tanto em planilhas
    normais quanto em modo ``read_only`` e evita acessos aleatórios repetidos.
    """
    ...
    cabecalhos = {
        ...
        for indice, valor in enumerate(cabecalho_linha, start=1)
    ...
    if faltando:
        encontrados = ", ".join(str(valor) for valor in cabecalho_linha)
        raise ValueError(
...
def carregar_tempos_cadastro(wb_valores) -> dict[str, tuple[float, float]]:
    """Lê os tempos técnicos da aba de cadastro de forma sequencial.

    Em modo ``read_only``, usar ``ws.cell`` repetidamente faz o openpyxl voltar
    várias vezes ao XML da planilha. ``iter_rows`` percorre cada linha apenas uma
    vez e é muito mais rápido em bases grandes.
    """
    ...
    cabecalho = next(
        ws_cad.iter_rows(min_row=1, max_row=1, values_only=True),
        (),
    )
    cabecalhos = {
        normalizar(valor): indice
    ...
    if col_codigo is None or col_setup is None or col_unitario is None:
        return {}

    max_col = max(col_codigo, col_setup, col_unitario) + 1
    lookup: dict[str, tuple[float, float]] = {}
    for valores in ws_cad.iter_rows(min_row=2, max_col=max_col, values_only=True):
        codigo = texto_limpo(valores[col_codigo] if col_codigo < len(valores) else None)
        if not codigo:
            continue
        setup = float_seguro(valores[col_setup] if col_setup < len(valores) else None)
        unitario = float_seguro(valores[col_unitario] if col_unitario < len(valores) else None)
...
def carregar_dados(caminho: Path, nome_aba: str) -> tuple[pd.DataFrame, dict[str, int]]:
    """Carrega a base principal com leitura sequencial e somente dos valores.

    Otimização 5.26:
    - abre o XLSX apenas uma vez (antes eram duas aberturas completas);
    - usa ``read_only=True`` para não materializar a pasta inteira em memória;
    - percorre a aba com ``iter_rows(values_only=True)`` em vez de dezenas de
      chamadas ``ws.cell`` por linha;
    - mantém o mesmo fallback de tempos técnicos quando fórmulas não têm cache.
    """
    ...
    try:
        if nome_aba not in wb_valores.sheetnames:
            raise ValueError(
                f'A aba "{nome_aba}" não existe. Abas encontradas: {", ".join(wb_valores.sheetnames)}'
            )

        ws_valores = wb_valores[nome_aba]
        col = localizar_colunas(ws_valores)
        tempos_cadastro = carregar_tempos_cadastro(wb_valores)
        max_coluna_necessaria = max(col.values())

        def valor_linha(valores: tuple[Any, ...], chave: str) -> Any:
            indice_excel = col.get(chave)
            if not indice_excel:
                return ""
            indice = indice_excel - 1
            return valores[indice] if indice < len(valores) else ""

        linhas: list[dict[str, Any]] = []
        for linha_excel, valores in enumerate(
            ws_valores.iter_rows(
                min_row=2,
                max_col=max_coluna_necessaria,
                values_only=True,
            ),
            start=2,
        ):
            lote = valor_linha(valores, "lote")
            setor = valor_linha(valores, "setor")
            qtd_prog = valor_linha(valores, "qtde_programada")
            if lote in (None, "") and setor in (None, "") and qtd_prog in (None, ""):
                continue

            data_prog_valor = valor_linha(valores, "data_programada")
            data_entrega_valor = valor_linha(valores, "data_entrega")
            data_real_valor = valor_linha(valores, "data_realizada")
            ...
            qtde_real = valor_linha(valores, "qtde_realizada")

            codigo_valor = valor_linha(valores, "codigo")
            setup_valor = valor_linha(valores, "setup_padrao")
            tempo_unitario_valor = valor_linha(valores, "tempo_unitario")
            tempo_programado_valor = valor_linha(valores, "tempo_programado")
            ...
            linhas.append(
                {
                    ...
                    "Cliente/Pedido": texto_limpo(valor_linha(valores, "cliente")),
                    "Produto/Equipamento": texto_limpo(valor_linha(valores, "produto")),
                    "Código Peça": texto_limpo(codigo_valor),
                    "Descrição Peça": texto_limpo(valor_linha(valores, "descricao")),
                    "Operação": texto_limpo(valor_linha(valores, "operacao")),
                    "Setor": texto_limpo(setor),
                    "Máquina/Posto": texto_limpo(valor_linha(valores, "maquina")),
                    ...
                    "Início Real": texto_limpo(valor_linha(valores, "inicio_real")),
                    "Fim Real": texto_limpo(valor_linha(valores, "fim_real")),
                    "Tempo Realizado (min)": numero_limpo(valor_linha(valores, "tempo_realizado")),
                    "Refugo/Retrabalho": numero_limpo(valor_linha(valores, "refugo_retrabalho")),
...
def carregar_paradas(caminho: Path) -> pd.DataFrame:
    """Carrega a aba de paradas em uma única passagem sequencial.

    Esta mudança é especialmente importante no ``read_only`` do openpyxl: acessar
    cada célula por ``ws.cell(row=...)`` pode fazer a planilha ser percorrida desde
    o início repetidas vezes. Em bases com centenas de paradas isso podia levar
    dezenas de segundos; com ``iter_rows`` a leitura é linear.
    """
    wb = load_workbook(
        caminho,
        data_only=True,
    ...
    try:
        ...
        max_coluna_necessaria = max(colunas.values())

        def valor_linha(valores: tuple[Any, ...], cabecalho: str) -> Any:
            indice_excel = colunas.get(cabecalho)
            if not indice_excel:
                return ""
            indice = indice_excel - 1
            return valores[indice] if indice < len(valores) else ""
        ...
        for linha, valores in enumerate(
            ...
            data_valor = valor_linha(valores, "Data Parada")
            setor = texto_limpo(valor_linha(valores, "Setor"))
            maquina = texto_limpo(valor_linha(valores, "Máquina/Posto"))
            motivo = texto_limpo(valor_linha(valores, "Motivo da Parada"))
            tempo_min = minutos_seguro(valor_linha(valores, "Tempo Total Parada (min)"))
            ...
            registros.append(
                {
                    ...
                    "Observação": texto_limpo(valor_linha(valores, "Observação")),
                    "Registrado Em": valor_linha(valores, "Registrado Em"),
                    "Usuário": texto_limpo(valor_linha(valores, "Usuário")),
                    "Editado Em": valor_linha(valores, "Editado Em"),
                    "Editado Por": texto_limpo(valor_linha(valores, "Editado Por")),
                }
            )
...
def renderizar_apontamento(df: pd.DataFrame, caminho: Path, nome_aba: str) -> None:
    ...
    # Grade enxuta do apontamento manual. Estas colunas continuam carregadas
    # internamente para filtros, gravação e relatórios, mas não ocupam espaço nesta tela:
    # Data Entrega, Máquina/Posto, Produto/Equipamento, Data Realizada,
    # Qtde Realizada e Situação.
    colunas_visiveis = [
        "Selecionar",
        "Data Programada",
        "OP/Lote",
...
# -----------------------------------------------------------------------------
# Exportação Excel da tela de programação
# -----------------------------------------------------------------------------
COLUNAS_EXCEL_PROGRAMADO_MAQUINA = [
    "Data",
    "Setor",
    "Máquina/Posto",
    "Unidade Métrica",
...
COLUNAS_EXCEL_APONTADO_MAQUINA = [
    "Data",
    "Setor",
    "Máquina/Posto",
    "Unidade Métrica",
...
def _largura_coluna_relatorio(cabecalho: str) -> float:
    """Larguras próximas ao modelo de Excel usado pelo PPCP."""
    larguras = {
        "Data": 14,
        "Setor": 29,
        "Máquina/Posto": 40,
        ...
        "Programado (peças)": 19,
        "Apontado (peças)": 19,
        "Lotes Programados": 30,
        "Lotes Apontados": 30,
        "Peças Distintas": 16,
    }
    return larguras.get(texto_limpo(cabecalho), 18)


def _formatar_bloco_excel_programacao(
    ws,
    df: pd.DataFrame,
    coluna_inicial: int,
    linha_cabecalho: int,
    ...
    """Formata um bloco no padrão visual azul do relatório enviado pelo PPCP."""
    if df is None:
        return

    primeira_coluna = int(coluna_inicial)
    ultima_coluna = primeira_coluna + max(len(df.columns) - 1, 0)
    ultima_linha = linha_cabecalho + len(df)

    preenchimento_cabecalho = PatternFill("solid", fgColor="002060")
    fonte_cabecalho = Font(color="FFFFFF", bold=True)

    for deslocamento, cabecalho in enumerate(df.columns):
        indice_coluna = primeira_coluna + deslocamento
        celula = ws.cell(row=linha_cabecalho, column=indice_coluna)
        celula.fill = preenchimento_cabecalho
        celula.font = fonte_cabecalho
        celula.alignment = Alignment(
            horizontal="center", vertical="center", wrap_text=True
        )
        ws.column_dimensions[get_column_letter(indice_coluna)].width = _largura_coluna_relatorio(
            texto_limpo(cabecalho)
        )

    ws.row_dimensions[linha_cabecalho].height = 40

    for linha in range(linha_cabecalho + 1, ultima_linha + 1):
        ...
        for deslocamento, cabecalho in enumerate(df.columns):
            ...
            celula.alignment = Alignment(
                horizontal=(
                    ...
                    if any(
                        termo in cabecalho_norm
                        for termo in ["data", "unidade", "programado", "apontado", "pecas distintas"]
                    )
                    else "left"
                ),
                vertical="center",
            ...
            if "data" in cabecalho_norm:
                celula.number_format = "dd/mm/yyyy"
            elif any(
                termo in cabecalho_norm
                for termo in ["programado pecas", "apontado pecas", "pecas distintas"]
            ):
                celula.number_format = "#,##0"

    # A Tabela do Excel cria os botões individuais de filtro no cabeçalho.
    # Sem TableStyleInfo, a cor aplicada manualmente permanece igual ao modelo.
    if len(df) > 0 and len(df.columns) > 0:
        referencia = (
            f"{get_column_letter(primeira_coluna)}{linha_cabecalho}:"
...
def excel_programado_maquina_formatado(programado: pd.DataFrame) -> bytes:
    """Exporta somente o programado no mesmo desenho de colunas do modelo enviado."""
    tabela = programado[COLUNAS_EXCEL_PROGRAMADO_MAQUINA].copy()
    saida = BytesIO()

    with pd.ExcelWriter(saida, engine="openpyxl") as writer:
        tabela.to_excel(writer, index=False, sheet_name="Programado por máquina")
        ws = writer.book["Programado por máquina"]
        ws.sheet_view.showGridLines = False
        ws.freeze_panes = "A2"
        _formatar_bloco_excel_programacao(
            ws,
            tabela,
            coluna_inicial=1,
            linha_cabecalho=1,
            nome_tabela="TabelaProgramadoMaquina",
        )
...
def excel_programado_apontado_lado_a_lado(
    programado: pd.DataFrame,
    apontado: pd.DataFrame,
    inicio: date,
    fim: date,
) -> bytes:
    """Gera uma única aba com Programado à esquerda e Apontado/Realizado à direita."""
    tabela_programado = programado[COLUNAS_EXCEL_PROGRAMADO_MAQUINA].copy()
    tabela_apontado = apontado[COLUNAS_EXCEL_APONTADO_MAQUINA].copy()
    saida = BytesIO()

    with pd.ExcelWriter(saida, engine="openpyxl") as writer:
        # Linha 1 fica reservada aos títulos dos dois blocos.
        tabela_programado.to_excel(
            writer,
            index=False,
            sheet_name="Programado x Apontado",
            startrow=1,
            startcol=0,
        )
        tabela_apontado.to_excel(
            writer,
            index=False,
            sheet_name="Programado x Apontado",
            startrow=1,
        ...
        ws = writer.book["Programado x Apontado"]
        ws.sheet_view.showGridLines = False
        ws.freeze_panes = "A3"
        ws.column_dimensions["H"].width = 3

        preenchimento_titulo = PatternFill("solid", fgColor="002060")
        fonte_titulo = Font(color="FFFFFF", bold=True, size=12)

        ws.merge_cells("A1:G1")
        ws["A1"] = f"PROGRAMADO — {inicio:%d/%m/%Y} a {fim:%d/%m/%Y}"
        ws["A1"].fill = preenchimento_titulo
        ws["A1"].font = fonte_titulo
        ws["A1"].alignment = Alignment(horizontal="center", vertical="center")

        ws.merge_cells("I1:O1")
        ws["I1"] = f"APONTADO / REALIZADO — {inicio:%d/%m/%Y} a {fim:%d/%m/%Y}"
        ws["I1"].fill = preenchimento_titulo
        ws["I1"].font = fonte_titulo
        ...
        _formatar_bloco_excel_programacao(
            ...
            coluna_inicial=1,
            linha_cabecalho=2,
            nome_tabela="TabelaProgramadoComparativo",
        )
        _formatar_bloco_excel_programacao(
            ws,
            tabela_apontado,
            coluna_inicial=9,
            linha_cabecalho=2,
            nome_tabela="TabelaApontadoComparativo",
        )
...
# -----------------------------------------------------------------------------
# Componentes visuais do dashboard de programação
# -----------------------------------------------------------------------------
def html_ranking_programacao(
    df: pd.DataFrame,
    titulo: str,
    coluna_nome: str,
    coluna_valor: str,
    cabecalho_valor: str,
    casas: int = 2,
    coluna_valor_secundario: str | None = None,
    cabecalho_valor_secundario: str | None = None,
    casas_secundario: int | None = None,
) -> str:
    """Monta o ranking visual e, opcionalmente, exibe uma segunda coluna de valor."""
    if df.empty:
        return (
            "<div style='padding:14px;border:1px solid #e2e8f0;border-radius:10px;'>"
            "Sem dados para o ranking.</div>"
        )

    tem_secundario = bool(coluna_valor_secundario and cabecalho_valor_secundario)
    casas_sec = casas if casas_secundario is None else casas_secundario

    linhas: list[str] = []
    for _, registro in df.head(10).iterrows():
        nome = escape(texto_limpo(registro.get(coluna_nome, "")) or "Sem identificação")
        valor = fmt_num(registro.get(coluna_valor, 0), casas)
        celula_secundaria = ""
        if tem_secundario:
            valor_secundario = fmt_num(
                registro.get(coluna_valor_secundario or "", 0),
                casas_sec,
            )
            celula_secundaria = (
                f"<td class='ranking-valor ranking-valor-secundario'>{valor_secundario}</td>"
            )

        linhas.append(
            "<tr>"
            f"<td class='ranking-nome'>{nome}</td>"
            f"<td class='ranking-valor'>{valor}</td>"
            f"{celula_secundaria}"
            "</tr>"
        )

    cabecalho_secundario_html = ""
    if tem_secundario:
        cabecalho_secundario_html = (
            f'<th class="ranking-cabecalho-valor ranking-cabecalho-secundario">'
            f'{escape(cabecalho_valor_secundario or "")}</th>'
        )

    classe_titulo = "ranking-titulo ranking-titulo-tres" if tem_secundario else "ranking-titulo"
    classe_valor = (
        "ranking-cabecalho-valor ranking-cabecalho-tres"
        if tem_secundario
        else "ranking-cabecalho-valor"
    )

    return f"""
    <style>
      .ranking-wrap {{
        width: 100%; overflow-x: auto; margin: .25rem 0 1rem 0;
      ...
      .ranking-table th, .ranking-table td {{
        border: 2px solid white; text-align: center; padding: 11px 14px;
      }}
      .ranking-table th {{
        font-size: 1.05rem; font-weight: 800; color: #111827;
      }}
      .ranking-titulo {{ background: #a8d08d; width: 76%; }}
      .ranking-titulo-tres {{ width: 60%; }}
      .ranking-cabecalho-valor {{ background: #ffc000; width: 24%; }}
      .ranking-cabecalho-tres {{ width: 20%; }}
      .ranking-cabecalho-secundario {{ background: #70ad47; width: 20%; }}
      .ranking-nome {{ background: #a8d08d; font-size: .96rem; color: #111827; }}
      .ranking-valor {{
        background: #ffc000; font-size: .96rem; color: #111827; white-space: nowrap;
      }}
      .ranking-valor-secundario {{ background: #70ad47; }}
      @media (max-width: 800px) {{
        .ranking-table th, .ranking-table td {{ padding: 8px 10px; font-size: .86rem; }}
      }}
    </style>
    <div class="ranking-wrap">
      <table class="ranking-table">
        <thead>
          <tr>
            <th class="{classe_titulo}">{escape(titulo)}</th>
            <th class="{classe_valor}">{escape(cabecalho_valor)}</th>
            {cabecalho_secundario_html}
          </tr>
        </thead>
        <tbody>{''.join(linhas)}</tbody>
...
def grafico_barras_horizontal(
    ...
    """Exibe barras horizontais e mantém compatibilidade com Streamlit mais antigo."""
    if df.empty:
        st.info("Sem dados para o gráfico.")
        return

    dados_grafico = df[[categoria, valor]].copy().set_index(categoria)
    try:
        st.bar_chart(dados_grafico, horizontal=True, height=altura)
    except TypeError:
        # Versões antigas do Streamlit não possuem o argumento horizontal.
        st.bar_chart(dados_grafico, height=altura)


# -----------------------------------------------------------------------------
# Tela pública: somente programado por máquina e acumulado por setor/dia
# -----------------------------------------------------------------------------
def renderizar_programado_maquina_setor_dia(df: pd.DataFrame, caminho: Path) -> None:
    """Exibe exclusivamente a programação, com ranking e gráficos gerenciais.

    A Data Programada é a referência temporal. Os mesmos registros alimentam a
    visão por máquina, o acumulado por setor/dia e o dashboard principal de quantidade programada.
    """
    st.title("Programado por Máquina e Acumulado por Setor/Dia")
    st.caption(
        "Tela somente de programação. A referência é a Data Programada da planilha; "
        "apontamentos, realizados e metas não entram nestes totais."
    )

    base = preparar_base_programado_equipamento(df)
    datas_validas = pd.to_datetime(base["Data Programada DT"], errors="coerce").dropna()
    if datas_validas.empty:
        st.warning("Não existem Datas Programadas válidas para montar esta tela.")
        return

    data_min = datas_validas.min().date()
    data_max = datas_validas.max().date()
    hoje = date.today()
    referencia_padrao = hoje if data_min <= hoje <= data_max else data_max
    ...
    if modo == "Dia específico":
        ...
        inicio = c_ini.date_input(
            ...
            max_value=data_max,
            format="DD/MM/YYYY",
            key="somente_prog_inicio",
        )
        fim = c_fim.date_input(
            "Data final",
            value=min(data_max, max(referencia_padrao, inicio)),
            min_value=data_min,
            max_value=data_max,
            format="DD/MM/YYYY",
            key="somente_prog_fim",
        )
        if fim < inicio:
    ...
    maquinas_sel = col_maquina.multiselect(
        "Máquina/Posto",
        maquinas,
        placeholder="Vazio = todas as máquinas",
        key="somente_prog_maquinas",
    )

    dados = base.copy()
    data_programada = pd.to_datetime(dados["Data Programada DT"], errors="coerce")
    mascara = (
        data_programada.notna()
        & (data_programada.dt.date >= inicio)
        & (data_programada.dt.date <= fim)
        & (pd.to_numeric(dados["Qtde Programada Num"], errors="coerce").fillna(0) > 0)
    )
    dados = dados[mascara].copy()
    if setores_sel:
        dados = dados[dados["Setor"].isin(setores_sel)]
    if maquinas_sel:
        dados = dados[dados["Máquina/Posto"].isin(maquinas_sel)]

    if dados.empty:
        st.warning("Não existe programação para o período e filtros selecionados.")
        return

    dados["Data"] = pd.to_datetime(dados["Data Programada DT"], errors="coerce").dt.date
    dados["Qtde Programada Num"] = pd.to_numeric(
        dados["Qtde Programada Num"], errors="coerce"
    ).fillna(0).clip(lower=0)
    dados["Métrica Programada Num"] = pd.to_numeric(
        dados["Métrica Programada Num"], errors="coerce"
    ).fillna(0).clip(lower=0)

    # O Tempo Programado já considera a regra existente do sistema/cadastro técnico.
    # Para o dashboard, apenas convertemos os minutos para horas.
    if "Tempo Programado (min)" in dados.columns:
        dados["Tempo Programado Num"] = (
            dados["Tempo Programado (min)"].map(float_seguro).fillna(0).clip(lower=0)
        )
    else:
        dados["Tempo Programado Num"] = 0.0
    dados["Horas Programadas"] = dados["Tempo Programado Num"] / 60.0
    ...
    por_maquina = (
        ...
        .agg(
            **{
                "Programado (peças)": ("Qtde Programada Num", "sum"),
                "Programado na unidade": ("Métrica Programada Num", "sum"),
                "Horas Programadas": ("Horas Programadas", "sum"),
                "Lotes Programados": ("OP/Lote", juntar_lotes),
                "Peças Distintas": ("Código Peça", "nunique"),
                "Linhas Programadas": ("Linha Excel", "count"),
            }
        )
        .sort_values(["Data", "Setor", "Máquina/Posto"])
    )

    por_setor_dia = (
        dados.groupby(
            ["Data", "Setor", "Unidade Métrica"],
            as_index=False,
            dropna=False,
        )
        .agg(
            **{
                ...
                "Programado na unidade": ("Métrica Programada Num", "sum"),
                "Horas Programadas": ("Horas Programadas", "sum"),
                "Máquinas Programadas": ("Máquina/Posto", "nunique"),
                "Lotes Programados": ("OP/Lote", juntar_lotes),
                "Peças Distintas": ("Código Peça", "nunique"),
                "Linhas Programadas": ("Linha Excel", "count"),
            }
        )
        .sort_values(["Data", "Setor"])
    )

    # Apontado/realizado do MESMO período selecionado.
    # A referência temporal aqui é Data Realizada (não Data Programada).
    dados_apontados = base.copy()
    data_realizada = pd.to_datetime(dados_apontados["Data Realizada DT"], errors="coerce")
    mascara_apontado = (
        data_realizada.notna()
        & (data_realizada.dt.date >= inicio)
        & (data_realizada.dt.date <= fim)
        & (pd.to_numeric(dados_apontados["Qtde Apontada Num"], errors="coerce").fillna(0) > 0)
    )
    dados_apontados = dados_apontados[mascara_apontado].copy()
    if setores_sel:
        dados_apontados = dados_apontados[dados_apontados["Setor"].isin(setores_sel)]
    if maquinas_sel:
        dados_apontados = dados_apontados[dados_apontados["Máquina/Posto"].isin(maquinas_sel)]

    if not dados_apontados.empty:
        dados_apontados["Data"] = pd.to_datetime(
            dados_apontados["Data Realizada DT"], errors="coerce"
        ).dt.date
        dados_apontados["Qtde Apontada Num"] = pd.to_numeric(
            dados_apontados["Qtde Apontada Num"], errors="coerce"
        ).fillna(0).clip(lower=0)

        por_maquina_apontado = (
            dados_apontados.groupby(
                ["Data", "Setor", "Máquina/Posto", "Unidade Métrica"],
                as_index=False,
                dropna=False,
            )
            .agg(
                **{
                    "Apontado (peças)": ("Qtde Apontada Num", "sum"),
                    "Lotes Apontados": ("OP/Lote", juntar_lotes),
                    "Peças Distintas": ("Código Peça", "nunique"),
                }
            )
        ...
        por_maquina_apontado = pd.DataFrame(columns=COLUNAS_EXCEL_APONTADO_MAQUINA)

    # Rankings consolidados no período selecionado.
    ranking_maquinas_horas = (
        dados.groupby(["Setor", "Máquina/Posto"], as_index=False, dropna=False)
        .agg(
            **{
                "Horas": ("Horas Programadas", "sum"),
                "Quantidade Programada": ("Qtde Programada Num", "sum"),
            }
        )
        .sort_values(["Horas", "Quantidade Programada"], ascending=[False, False])
    )

    # Soma o total apontado por equipamento no mesmo período selecionado.
    # A programação continua usando Data Programada; o apontado usa Data Realizada.
    if dados_apontados.empty:
        ranking_maquinas_horas["Quantidade Apontada"] = 0.0
    else:
        ranking_maquinas_apontado = (
            dados_apontados.groupby(
                ["Setor", "Máquina/Posto"],
                as_index=False,
                dropna=False,
            ...
            .sum()
            .rename(columns={"Qtde Apontada Num": "Quantidade Apontada"})
        )
        ranking_maquinas_horas = ranking_maquinas_horas.merge(
            ranking_maquinas_apontado,
            on=["Setor", "Máquina/Posto"],
            how="left",
        )
        ranking_maquinas_horas["Quantidade Apontada"] = pd.to_numeric(
            ranking_maquinas_horas["Quantidade Apontada"], errors="coerce"
        ).fillna(0).clip(lower=0)

    # Quando o mesmo texto de máquina aparece em setores diferentes, acrescenta o setor
    # somente nesses casos para o ranking não consolidar visualmente equipamentos distintos.
    nomes_maquinas = ranking_maquinas_horas["Máquina/Posto"].fillna("").astype(str)
    duplicados = nomes_maquinas.duplicated(keep=False)
    ranking_maquinas_horas["Máquina"] = nomes_maquinas
    ranking_maquinas_horas.loc[duplicados, "Máquina"] = (
        nomes_maquinas[duplicados]
        + " — "
        + ranking_maquinas_horas.loc[duplicados, "Setor"].fillna("").astype(str)
    )

    ranking_maquinas_qtd = ranking_maquinas_horas.sort_values(
        ["Quantidade Programada", "Horas"], ascending=[False, False]
    ).copy()

    ranking_setores_horas = (
        dados.groupby("Setor", as_index=False, dropna=False)
        .agg(
            **{
                "Horas": ("Horas Programadas", "sum"),
                "Quantidade Programada": ("Qtde Programada Num", "sum"),
                "Máquinas": ("Máquina/Posto", "nunique"),
            }
        )
        .sort_values(["Horas", "Quantidade Programada"], ascending=[False, False])
    )
    ...
    horas_por_dia = (
        dados.groupby("Data", as_index=False)["Horas Programadas"]
        .sum()
        .sort_values("Data")
        .rename(columns={"Horas Programadas": "Horas"})
    )
    horas_por_dia["Dia"] = horas_por_dia["Data"].map(
        lambda valor: valor.strftime("%d/%m/%Y") if isinstance(valor, date) else texto_limpo(valor)
    )

    matriz_setor = por_setor_dia.pivot_table(
        index=["Setor", "Unidade Métrica"],
        columns="Data",
        values="Programado na unidade",
        aggfunc="sum",
        fill_value=0,
    )
    matriz_setor = matriz_setor.sort_index()
    matriz_setor["Total do período"] = matriz_setor.sum(axis=1)
    matriz_setor = matriz_setor.reset_index()
    matriz_setor.columns = [
        coluna.strftime("%d/%m/%Y") if isinstance(coluna, date) else str(coluna)
        for coluna in matriz_setor.columns
    ]

    total_pecas = float(dados["Qtde Programada Num"].sum())
    total_horas = float(dados["Horas Programadas"].sum())
    total_setores = int(dados["Setor"].nunique())
    total_maquinas = int(dados[["Setor", "Máquina/Posto"]].drop_duplicates().shape[0])
    total_dias = int(dados["Data"].nunique())

    st.divider()
    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("Programado (peças)", fmt_num(total_pecas, 2))
    k2.metric("Horas programadas", f"{fmt_num(total_horas, 2)} h")
    k3.metric("Setores", fmt_num(total_setores))
    k4.metric("Máquinas/Postos", fmt_num(total_maquinas))
    k5.metric("Dias com programação", fmt_num(total_dias))

    resumo_unidades = (
        dados.groupby("Unidade Métrica", dropna=False)["Métrica Programada Num"]
        .sum()
        .reset_index()
    )
    resumo_texto = " | ".join(
        f"{texto_limpo(linha['Unidade Métrica']) or 'sem unidade'}: "
        f"{fmt_num(linha['Métrica Programada Num'], 2)}"
        for _, linha in resumo_unidades.iterrows()
    )
    if resumo_texto:
        st.info(f"Programado por unidade produtiva — {resumo_texto}")

    aba_dashboard, aba_maquina, aba_setor, aba_matriz, aba_linhas = st.tabs(
        [
            "📊 Dashboard",
            "Programado por máquina",
            "Acumulado por setor/dia",
            "Matriz setor x dia",
            "Linhas programadas",
        ]
    )
    ...
    with aba_dashboard:
        st.subheader("Dashboard da programação")
        st.caption(
            "Ranking principal calculado pela quantidade programada da planilha. "
            "A nova coluna mostra o total de peças apontadas em cada equipamento no mesmo período, "
            "considerando a Data Realizada."
        )

        top10_qtd = ranking_maquinas_qtd.head(10).copy()
        st.markdown(
            html_ranking_programacao(
                top10_qtd,
                titulo="Top 10 Máquinas",
                coluna_nome="Máquina",
                coluna_valor="Quantidade Programada",
                cabecalho_valor="Peças programadas",
                casas=0,
                coluna_valor_secundario="Quantidade Apontada",
                cabecalho_valor_secundario="Peças apontadas",
                casas_secundario=0,
            ),
            unsafe_allow_html=True,
        )
        ...
        with col_g1:
            st.markdown("#### Top 10 máquinas — quantidade programada")
            grafico_barras_horizontal(
                top10_qtd.sort_values("Quantidade Programada", ascending=True),
                "Máquina",
                "Quantidade Programada",
                altura=430,
            )
        with col_g2:
            st.markdown("#### Top 10 máquinas — horas programadas")
            top10_horas = ranking_maquinas_horas.head(10).copy()
            grafico_barras_horizontal(
                top10_horas.sort_values("Horas", ascending=True),
                "Máquina",
                "Horas",
                altura=430,
            )
        ...
        with col_s1:
            st.markdown("#### Top 10 setores — quantidade programada")
            top10_setores_qtd = ranking_setores_horas.sort_values(
                ["Quantidade Programada", "Horas"], ascending=[False, False]
            ).head(10).copy()
            st.markdown(
                html_ranking_programacao(
                    top10_setores_qtd,
                    titulo="Top 10 Setores",
                    coluna_nome="Setor",
                    coluna_valor="Quantidade Programada",
                    cabecalho_valor="Peças",
                    casas=0,
                ),
                unsafe_allow_html=True,
            )
        with col_s2:
            st.markdown("#### Quantidade programada por setor")
            grafico_barras_horizontal(
                top10_setores_qtd.sort_values("Quantidade Programada", ascending=True),
                "Setor",
                "Quantidade Programada",
                altura=430,
            )
        ...
        st.divider()
        st.markdown("#### Evolução da quantidade programada por dia")
        quantidade_por_dia = (
            dados.groupby("Data", as_index=False)["Qtde Programada Num"]
            .sum()
            .sort_values("Data")
            .rename(columns={"Qtde Programada Num": "Quantidade Programada"})
        )
        quantidade_por_dia["Dia"] = quantidade_por_dia["Data"].map(
            lambda valor: valor.strftime("%d/%m/%Y") if isinstance(valor, date) else texto_limpo(valor)
        )
        if not quantidade_por_dia.empty:
            st.bar_chart(
                quantidade_por_dia[["Dia", "Quantidade Programada"]].set_index("Dia"),
                height=360,
            )

    with aba_maquina:
        st.subheader("Programado por máquina")
        st.caption(
            "Uma linha por Data Programada + Setor + Máquina/Posto. "
            "O campo 'Programado na unidade' respeita a unidade produtiva do setor."
        )
        tabela_maquina = por_maquina.copy()
        tabela_maquina["Data"] = tabela_maquina["Data"].map(
            lambda valor: valor.strftime("%d/%m/%Y") if isinstance(valor, date) else texto_limpo(valor)
        )
        st.dataframe(
            tabela_maquina,
            ...
            height=580,
            column_config={
                "Máquina/Posto": st.column_config.TextColumn("Máquina/Posto", width="large"),
                "Programado (peças)": st.column_config.NumberColumn("Programado (peças)", format="%.2f"),
                "Programado na unidade": st.column_config.NumberColumn("Programado na unidade", format="%.3f"),
                "Horas Programadas": st.column_config.NumberColumn("Horas programadas", format="%.2f"),
                "Lotes Programados": st.column_config.TextColumn("Lotes Programados", width="large"),
            },
        )

    with aba_setor:
        st.subheader("Acumulado por setor em cada dia")
        st.caption(
            "Soma todas as máquinas/postos pertencentes ao setor no mesmo dia programado."
        )
        tabela_setor = por_setor_dia.copy()
        tabela_setor["Data"] = tabela_setor["Data"].map(
            lambda valor: valor.strftime("%d/%m/%Y") if isinstance(valor, date) else texto_limpo(valor)
        )
        st.dataframe(
            tabela_setor,
    ...
    with aba_matriz:
        st.subheader("Matriz do programado por setor e por dia")
        st.caption(
            "As colunas são as Datas Programadas. Cada célula mostra o total programado "
            "na unidade produtiva do setor (uni, M² ou metro linear)."
        )
        st.dataframe(
            matriz_setor,
            use_container_width=True,
    ...
    with aba_linhas:
        st.subheader("Linhas da programação consideradas")
        colunas_detalhe = [
            "Data Programada",
            "Setor",
            "Máquina/Posto",
            ...
            "Horas Programadas",
            "Unidade Métrica",
            "Métrica Programada Num",
        ]
        detalhe = dados[[c for c in colunas_detalhe if c in dados.columns]].copy()
        detalhe = detalhe.rename(columns={"Métrica Programada Num": "Programado na unidade"})
        st.dataframe(
            detalhe,
            use_container_width=True,
            hide_index=True,
    ...
    # Exportações: o primeiro arquivo preserva o desenho de colunas do modelo enviado.
    # O segundo coloca Programado e Apontado/Realizado na mesma aba, lado a lado.
    tabela_export_programado = por_maquina[COLUNAS_EXCEL_PROGRAMADO_MAQUINA].copy()
    tabela_export_apontado = por_maquina_apontado[COLUNAS_EXCEL_APONTADO_MAQUINA].copy()

    col_xlsx_maquina, col_xlsx_comparativo, col_xlsx_setor = st.columns(3)
    col_xlsx_maquina.download_button(
        "Baixar programado por máquina",
        data=excel_programado_maquina_formatado(tabela_export_programado),
        file_name=f"programado_por_maquina_{inicio:%Y%m%d}_{fim:%Y%m%d}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key="download_somente_prog_maquina",
    )
    col_xlsx_comparativo.download_button(
        "Baixar Programado + Apontado",
        data=excel_programado_apontado_lado_a_lado(
            tabela_export_programado,
            tabela_export_apontado,
            inicio,
            fim,
        ),
        file_name=f"programado_x_apontado_{inicio:%Y%m%d}_{fim:%Y%m%d}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key="download_prog_apontado_mesma_planilha",
        help=(
            "Na mesma aba: Programado à esquerda pela Data Programada e "
            "Apontado/Realizado à direita pela Data Realizada, no mesmo período selecionado."
        ),
    )
    col_xlsx_setor.download_button(
        "Baixar acumulado setor/dia",
        data=dataframe_para_excel(tabela_setor, "Setor por dia"),
        file_name=f"programado_setor_dia_{inicio:%Y%m%d}_{fim:%Y%m%d}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key="download_somente_prog_setor",
    )

    st.caption(
        f"Fonte dos dados: {caminho.name} — Programado pela Data Programada e Apontado pela Data Realizada."
    )
...
def main() -> None:
    ...
    mapa_parametros = {
        ...
        "equipamentos": 0,
        "programado-maquina-setor": 1,
        "programado-maquina": 1,
        "programado-setor-dia": 1,
        "dashboard-programacao": 1,
        "dashboard-maquinas": 1,
        "acumulado-setor": 2,
        "acumulado-diario-setor": 2,
        "metas-setor": 2,
        "calendario": 3,
        "semanal": 3,
        "consulta-paradas": 4,
        "paradas-leitura": 4,
        "apontamento": 5,
        "producao-apontamento": 5,
        "paradas": 6,
        "apontamento-paradas": 6,
    }
    pagina_padrao = mapa_parametros.get(pagina_param, 0)

    paginas = [
        "🏭 Programado por Dia/Equipamento — leitura",
        "📊 Programado por Máquina + Setor/Dia + Dashboard — leitura",
    ...
    elif "Programado por Máquina + Setor/Dia" in pagina:
        renderizar_programado_maquina_setor_dia(df, caminho)
